...
@section('content')
    <div class="max-w-3xl mx-auto transition-colors">
        <div class="mb-8">
            ...
            <h1 class="mt-4 text-3xl font-bold text-slate-900 dark:text-slate-100 transition-colors">Perbarui Produk</h1>
            <p class="text-slate-500 dark:text-slate-400 transition-colors">Anda sedang mengubah data untuk produk <span
                    class="font-bold text-slate-900 dark:text-slate-200 transition-colors transition">#{{ $product->id_produk }}</span>.
            </p>
        </div>
        ...
        <div
            ...
            <form action="{{ route('products.update', $product->id_produk) }}" method="POST" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <span
                                    class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 font-bold text-sm transition-colors transition">Rp</span>
                            ...
                            <select name="status_id" id="status_id"
                                ...
                                @foreach ($statuses as $status)
                                    <option value="{{ $status->id_status }}"
                                        {{ old('status_id', $product->status_id) == $status->id_status ? 'selected' : '' }}>
